...
class CloudflareMonitor:
    ...
    def capture_screenshot(self, max_retries=3, timeout=15):
        ...
        for attempt in range(max_retries):
            try:
                ...
                subprocess.run(
                    vncdo_cmd, check=True, capture_output=True, timeout=timeout
                )
    ...
    def calculate_click_position(self, bbox):
        ...
        logger.info(
            f"计算点击位置: logo位置({x1},{y1})-({x2},{y2}) -> 点击位置({click_x},{click_y})"
        )
        ...
        try:
            ...
            for config, img_to_use in configs_and_images:
                try:
                    text = pytesseract.image_to_string(
                        img_to_use, config=config, lang="eng"
                    )
            ...
            if "imnotarobot" in text_clean:
                ...
                if has_registration and has_recaptcha:
                    ...
                    # 不在注册界面，验证应该已完成
                    logger.info(
                        "✅ OCR检测到'I'm not a robot'但已离开注册界面，验证已通过"
                    )
                    return "success"
        ...
        for attempt in range(max_retries):
            ...
            if OCR_AVAILABLE:
                ...
                if status == "success":
                    ...
                    # 点击重新开始按钮
                    logger.info(
                        f"点击重新开始按钮: ({retry_button_x}, {retry_button_y})"
                    )
                    if self.move_mouse_and_wait(
                        retry_button_x, retry_button_y, wait_time=1
                    ):
                        if self.click_at_current_position():
                    ...
                    try:
                        ...
                        text = pytesseract.image_to_string(
                            img_gray, config=r"--oem 3 --psm 6", lang="eng"
                        )
                        text_lower = text.lower()

                        if (
                            "pressplaytolisten" in text_lower.replace(" ", "")
                            or "press play to listen" in text_lower
                        ):
                        ...
                        if not still_detected:
                            ...
                            if OCR_AVAILABLE:
                                ...
                                if status == "success":
                                    ...
                                    logger.info("🔄 OCR检测到验证挑战，开始语音验证...")
                                    click_x, click_y = 735, 985
                                    success = self.handle_voice_verification_retry(
                                        click_x, click_y
                                    )
                                else:
                                    logger.info("🔄 OCR状态未明确，尝试语音验证...")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--interval", type=int, default=3, help="检测间隔（秒），默认为3秒"
    )
    parser.add_argument(
        "--wait", type=int, default=5, help="点击后等待验证的时间（秒），默认为5秒"
    )
    parser.add_argument(
        "--voice-timeout",
        type=int,
        default=30,
        help="谷歌语音验证检测超时时间（秒），默认为30秒",
    )
    parser.add_argument(
        "--debug", action="store_true", help="启用调试模式，保存截图并显示详细信息"
    )
    parser.add_argument(
        "--voice-only", action="store_true", help="仅检测谷歌语音按钮（调试模式）"
    )
    parser.add_argument(
        "--move-to",
        type=str,
        help="移动鼠标到指定坐标（格式：x,y）供调试用，不执行点击",
    )
    ...
    if args.voice_only:
        ...
        monitor.run_voice_debug_only(
            check_interval=args.interval, voice_timeout=args.voice_timeout
        )
    else:
